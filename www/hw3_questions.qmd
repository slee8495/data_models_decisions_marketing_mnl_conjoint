...
yogurt_shares %>% 
  ...
  labs(title = "Market Shares for Yogurt Products before Inreasing the Price by $0.1", x = "Yogurt", y = "Market Share (%)") +
  theme_classic() +
...
yogurt_shares_2 %>% 
  ...
  geom_text(aes(label = sprintf("%.2f%%", MarketShare)), vjust = -0.3, size = 5) +
  scale_fill_manual(values = custom_colors, guide = "none") + 
  labs(title = "Market Shares for Yogurt Products after Increasing the Price by $0.1", x = "Yogurt", y = "Market Share (%)") +
  theme_classic() +
  theme(text = element_text(size = 15))
```
...
p1 <- ggplot(data1, aes(x = Yogurt, y = MarketShare, fill = Yogurt == "Yogurt 1")) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  geom_text(aes(label = paste0(MarketShare, "%")), vjust = -0.5) +
  scale_fill_manual(values = c("gray", "lightgreen")) +
  labs(title = "Market Shares Changes for Yogurt Products by Increating the price by $0.1", y = "Market Share (%)") +
  theme_classic()

# Create the second graph
p2 <- ggplot(data2, aes(x = Yogurt, y = MarketShare, fill = Yogurt == "Yogurt 1")) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  geom_text(aes(label = paste0(MarketShare, "%")), vjust = -0.5) +
...
arrow <- grid::linesGrob(arrow = grid::arrow(type = "closed", length = unit(0.2, "inches")),
                         gp = grid::gpar(col = "black"), x = unit(c(0.25, 0.75), "npc"), y = unit(c(0.5, 0.5), "npc"))

# Arrange the graphs and the arrow
